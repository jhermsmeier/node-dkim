...
function processHeader(headers, signHeaders, method) {
  ...
  if (signHeaders != null) {
    ...
    let indexedHeaders = headers.map(function (header) {
      var key = header.slice(0, header.indexOf(':')).trim().toLowerCase()
      let idx = signHeaders.indexOf(key)
      return { idx, header }
    })
    headers = indexedHeaders.filter(h => h.idx > -1).sort((h1, h2) => h1.idx - h2.idx).map(h => h.header)
