  ...
  header.split( /\r\n(?=[^\x20\x09]|$)/g ).forEach( function( h, i, headers ) {
    if( /^(DKIM-Signature|X-Google-DKIM-Signature)/i.test( h ) ) {
      ...
      // FIX: after slicing, remove any included 'DKIM-Signature' header that differ from "oneHeader"
       headers.slice( i )
       var sigHeaders = processSignature(headers.slice( i ), h)
...
function processSignature(headers, signatureHeader) {
    ...
    var sigHeaders = headers.filter(function(h){ return h === signatureHeader || /^(?!(DKIM-Signature|X-Google-DKIM-Signature))/i.test(h); } )
